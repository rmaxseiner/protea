...
## Deployment Planning Notes (for next session)

### Current Setup
- **Python:** 3.11+ required
- **Web UI Port:** 8080 (configurable via `INVENTORY_WEB_PORT`)
- **Database:** SQLite at `/mnt/data/work/PycharmProjects/inventory-mcp/data/inventory.db`
- **Images:** Stored at `/mnt/data/work/PycharmProjects/inventory-mcp/data/images/`
- **Venv:** `/mnt/data/work/PycharmProjects/inventory-mcp/.venv`

### Required Environment Variables
```bash
INVENTORY_DATABASE_PATH=/path/to/inventory.db
INVENTORY_IMAGE_BASE_PATH=/path/to/images
INVENTORY_CLAUDE_API_KEY=sk-ant-...  # For vision extraction
```

### Two Services to Deploy
1. **inventory-web** - Web UI (FastAPI/uvicorn) - user-facing
2. **inventory-mcp** - MCP server for Claude Desktop - stdio-based

### Deployment Questions to Consider
- Single machine or separate server?
- Docker vs systemd service?
- Need external access or LAN-only?
- Existing reverse proxy (nginx/caddy/traefik)?
- SSL certificates (Let's Encrypt)?
- Backup strategy for database/images?
