...
**New Files:**
- `src/protea/services/embedding_service.py` - Embedding generation & similarity
- `src/protea/db/migrations/004_item_embeddings.sql` - Adds embedding column
- `src/protea/backfill_embeddings.py` - CLI backfill tool
...
**Files Changed:**
- `src/protea/web/templates/base.html` - All static asset URLs
...
**Files:**
- `src/protea/web/static/favicon.svg` - 32x32 favicon
- `src/protea/web/static/protea-icon.svg` - 64x64 detailed version
...
┌─────────────────────────────────────────────────────────────┐
│                    Docker Container                          │
│            git.maxseiner.casa/ron-maxseiner/protea   │
├─────────────────────────────────────────────────────────────┤
│                                                              │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐       │
│  │ inventory-web│  │protea │  │protea │       │
│  │   (web UI)   │  │   (stdio)    │  │    -sse      │       │
...
| `src/protea/config.py` | Embedding configuration settings |
| `src/protea/services/__init__.py` | Export embedding_service |
| `src/protea/services/embedding_service.py` | NEW: Vector embeddings |
| `src/protea/db/migrations/004_item_embeddings.sql` | NEW: Embedding column |
| `src/protea/tools/items.py` | Generate embeddings on create/update |
| `src/protea/tools/search.py` | Hybrid FTS + vector search |
| `src/protea/backfill_embeddings.py` | NEW: Backfill CLI |
| `src/protea/web/templates/base.html` | Relative URLs, protea icon |
| `src/protea/web/templates/item.html` | Action buttons and forms |
| `src/protea/web/templates/bin.html` | Sub-bin management |
| `src/protea/web/static/app.js` | Action toggles, lightbox fixes |
| `src/protea/web/static/favicon.svg` | NEW: King Protea favicon |
| `src/protea/web/static/protea-icon.svg` | NEW: King Protea icon |
| `src/protea/web/routes/pages.py` | Item action routes, sub-bin routes |
